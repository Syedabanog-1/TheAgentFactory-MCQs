...
export const chapter15: Chapter = {
  ...
  ,
    {
      id: 11,
      title: "Scenario-Based Mastery — Context Engineering",
      mcqs: [
        { id: 1, question: "An AI agent keeps repeating the same architectural mistake despite being corrected multiple times. Each session starts fresh. What context engineering solution addresses this?", options: { A: "Use a more powerful AI model that remembers corrections", B: "Encode the corrected constraint in CLAUDE.md — persistent project context that loads automatically every session, converting repeated corrections into a permanent structural constraint the agent applies from the start", C: "Use a longer system prompt with detailed instructions", D: "Run multiple sessions in parallel to reduce error frequency" }, correct: "B", explanation: "Repeated corrections signal missing persistent context. CLAUDE.md encodes constraints that survive session boundaries — once documented, the architectural constraint is present from session start without manual re-specification. This is context engineering's core value: transforming tacit corrections (said once, forgotten next session) into explicit, persistent context (written once, applied every session)." },
        { id: 2, question: "Your context window is 80% full with debugging history from earlier in a session. You need to start a complex new feature implementation. What context lifecycle decision should you make?", options: { A: "Continue in the same session — more context is always better", B: "Reset to a clean session — the debugging history is noise for the new feature. Clean slates beat dirty states: irrelevant prior context degrades AI performance by filling attention with non-signal content", C: "Compress the context by summarizing the debug history into one paragraph", D: "Switch to a larger context window model and continue" }, correct: "B", explanation: "Context lifecycle: when a task changes significantly, prior context becomes noise. 80% full with debugging history means only 20% available for the new complex feature — insufficient signal-to-noise ratio. Reset to a clean session with only the relevant CLAUDE.md and feature specification. Chapter 15's principle: 'Context Isolation — Why Clean Slates Beat Dirty States' — old session state creates unpredictable interference." },
        { id: 3, question: "A team member has 10 years of implicit knowledge about the company's legacy codebase. When they leave, that knowledge becomes unavailable to AI agents. Which Chapter 15 technique addresses this?", options: { A: "Train a custom AI model on the codebase", B: "Extract tacit knowledge through structured interviews and encode it in CLAUDE.md and specification documents — the Two-Way Problem solution: getting expert implicit knowledge out of human heads and into persistent AI context", C: "Hope the AI model infers the knowledge from reading the code", D: "Hire a replacement developer who re-learns the same implicit knowledge" }, correct: "B", explanation: "The Two-Way Problem: getting tacit knowledge from human experts into AI context. Structured knowledge extraction interviews surface implicit understanding (why certain patterns exist, what edge cases to avoid, undocumented constraints). Encoding this in CLAUDE.md makes it available to AI agents going forward. Chapter 15's technique: interview the expert before they leave, document in structured Markdown, review for completeness." },
        { id: 4, question: "You're working on a 3-week project with 15 implementation phases. Midway through, you start a new session and Claude has no memory of prior decisions. What architecture should you have used?", options: { A: "Keep all work in a single session to avoid context loss", B: "Progress file architecture — a structured Markdown file tracking decisions made, current state, next steps, and blockers. Sessions start by reading the progress file, maintaining continuity across session boundaries without relying on session memory", C: "Use a database to store all session decisions", D: "Send a context summary email to yourself after each session" }, correct: "B", explanation: "Long-horizon work requires progress files: a structured PROGRESS.md documenting phase completion, key decisions, open questions, and current state. Each new session reads the progress file to reconstruct context in minutes. Chapter 15's principle: for work spanning multiple sessions, persist state in files, not session memory — files survive context resets, session memory doesn't." },
        { id: 5, question: "An AI agent's context contains: 15 error messages from 3 days ago, 8 abandoned code attempts, and 3 currently relevant files. What should you do before asking for a new feature?", options: { A: "Add the new feature request — AI will figure out what's relevant", B: "Audit and clean: remove the error messages and abandoned attempts (noise), keep only the 3 relevant files and new feature spec. Signal vs. Noise auditing ensures AI attention focuses on relevant information, not historical clutter", C: "Archive the old context to a file and reference it if needed", D: "Summarize all previous work in a single paragraph at the top of the context" }, correct: "B", explanation: "Signal vs. Noise audit: 15 old error messages and 8 abandoned attempts are pure noise for a new feature. They consume context space and attention without contributing to the new task. Clean context before critical work: remove irrelevant history, keep only what the AI needs for the current task. Chapter 15's audit framework: classify each context item as signal (needed now) or noise (historical artifact), remove noise before important tasks." },
        { id: 6, question: "You're implementing a large feature and need to inject a critical security requirement mid-session that was just communicated by the security team. The session is deep in implementation. What's the correct approach?", options: { A: "Start a new session with the security requirement", B: "Mid-stream memory injection — add the security requirement as an explicit message in the current context, clearly marked as a new constraint. The agent incorporates it going forward without losing existing implementation progress", C: "Note the requirement and apply it in a code review after implementation", D: "Security requirements cannot be added mid-session" }, correct: "B", explanation: "Mid-stream injection: inject the constraint explicitly into the current session context with clear framing ('IMPORTANT NEW CONSTRAINT: [security requirement]'). The agent reads the injection and applies it from that point forward without restarting. Chapter 15's technique: when critical context must be added during active work, explicit injection is more effective than hoping the agent infers the new constraint from subtle cues." },
        { id: 7, question: "Two developers work on the same feature but get completely different implementations from Claude. Both gave similar requirements. What context engineering factor likely explains the difference?", options: { A: "Random variation — AI models always produce different outputs", B: "Different context quality: different CLAUDE.md configurations, different prior session histories contaminating context, or different specificity in requirements documents — context composition determines output consistency", C: "They used different Claude model versions", D: "Time of day affects AI performance" }, correct: "B", explanation: "Context architecture determines output consistency: if two developers have identical requirements but different context (one has relevant CLAUDE.md constraints, prior decisions, and clear spec; the other has accumulated noise and vague requirements), the outputs will differ significantly. Chapter 15's insight: inconsistent AI output is often a context quality problem, not a model problem. Standardize context architecture → standardize output quality." },
        { id: 8, question: "What is the Tasks system's role in context engineering for long-running agent work?", options: { A: "It is a project management tool for human team coordination", B: "The Tasks system provides structured persistent state — a formal mechanism for AI agents to track work items, mark completions, and communicate progress across session boundaries with clear status semantics", C: "Tasks automatically delegate work to subagents", D: "Tasks is only available in the enterprise version of Claude Code" }, correct: "B", explanation: "The Tasks system creates persistent, structured state for agent work: tasks have status (pending/in-progress/done), can be read at session start to reconstruct context, and provide unambiguous progress tracking across sessions. Chapter 15 introduces tasks as the mechanism for encoding work state that survives context resets and enables reliable long-horizon agent execution." },
        { id: 9, question: "A developer complains that 'Claude keeps doing things I told it not to do in previous sessions.' What is the most likely context engineering failure?", options: { A: "The Claude model has a bug that ignores negative instructions", B: "Constraints communicated only in session memory (not in CLAUDE.md) are lost at session end — the developer is re-correcting the same behavior because the constraint was never made persistent. Fix: encode the constraint in CLAUDE.md", C: "The developer needs to use a stronger tone when correcting Claude", D: "This behavior is normal and cannot be prevented" }, correct: "B", explanation: "Session memory is ephemeral: corrections made during a session die when the session ends. Repeated corrections = missing persistent context. Chapter 15 solution: when you correct Claude's behavior and want it to persist, immediately encode the constraint in CLAUDE.md. Convert ephemeral session corrections to permanent project constraints. 'Never do X' in a session is a note; 'Never do X' in CLAUDE.md is a rule." },
        { id: 10, question: "You're building a context architecture for a complex 6-month project. What should be in CLAUDE.md vs. what should be in a progress file?", options: { A: "Everything should be in CLAUDE.md — single source of truth", B: "CLAUDE.md: stable constraints (conventions, architecture principles, key commands, patterns to follow). Progress file: dynamic state (current phase, completed tasks, open decisions, next steps). CLAUDE.md is stable; progress files are current-state snapshots", C: "CLAUDE.md is for technical content; progress files are for project management", D: "Both files serve the same purpose — use whichever feels more natural" }, correct: "B", explanation: "Context architecture separates stable from dynamic: CLAUDE.md holds project invariants (won't change — coding conventions, architecture decisions, security constraints). Progress files hold dynamic state (changes daily — current phase, pending decisions, completed items, blockers). Loading both at session start gives Claude stable constraints + current state = complete context without confusion." },
        { id: 11, question: "An AI agent is working on a 5-step implementation. After step 2, you need to add a constraint that affects steps 3-5. The agent has already started step 3 without the constraint. What should you do?", options: { A: "Let the agent finish all steps, then apply the constraint in a review pass", B: "Interrupt with explicit mid-stream injection: clearly state the new constraint and ask the agent to apply it immediately, even if it means revising in-progress work. Earlier injection prevents compounding errors across remaining steps", C: "Start entirely over from step 1 with the constraint from the beginning", D: "Add the constraint to CLAUDE.md and restart the session" }, correct: "B", explanation: "Mid-stream injection prevents error propagation: if a constraint must apply to steps 3-5, inject it immediately at the earliest detection point — even mid-step-3. The cost of correction grows with each additional step built without the constraint. Chapter 15: inject critical new context immediately when identified, clearly framed. Don't let constraint violations compound — the further you progress without correction, the more expensive the fix." },
        { id: 12, question: "What makes 'context isolation' (clean slates) superior to 'context accumulation' (keeping everything) for complex AI tasks?", options: { A: "Clean slates are always faster because they use fewer tokens", B: "Accumulated context contains noise from prior tasks that doesn't apply to new work — filling attention capacity, introducing false associations, and causing AI to apply prior task constraints incorrectly to new contexts. Isolation ensures each task gets full attention on relevant information only", C: "Accumulated context causes security vulnerabilities", D: "AI models have a hard limit on accumulated context that causes crashes" }, correct: "B", explanation: "Context isolation value: when prior task history accumulates, it creates attention pollution. The AI optimizing a payment system may incorrectly apply constraints from earlier authentication work if both are in context. Clean slate ensures the task gets full uncontaminated attention. Chapter 15's principle: complex new tasks deserve fresh context — start with CLAUDE.md plus task-specific materials only, not accumulated history." },
        { id: 13, question: "A team builds a context engineering playbook. What should the decision framework's first question be when starting any new AI-assisted task?", options: { A: "'What model should I use?'", B: "'What context does this task actually need?' — identify the minimum necessary context (relevant CLAUDE.md sections, spec, current state) and strip everything else. Right-sized context beats maximum context for quality and consistency", C: "'How long will this task take?'", D: "'Should I use a Skill or Subagent?'" }, correct: "B", explanation: "Context quality framework starts with minimum necessary context: identify what the task genuinely requires, then construct context accordingly. Over-loading context (including everything possibly relevant) degrades performance through noise. Under-loading (omitting actually needed constraints) causes errors. Chapter 15's playbook: ask 'what does this task need?' and include exactly that — CLAUDE.md for stable constraints, current spec, relevant existing code, and nothing extraneous." },
        { id: 14, question: "A developer says 'I just tell Claude everything at the start of each session and it works fine.' Why might this approach fail at scale?", options: { A: "It always works — this is the recommended approach", B: "Manual context verbalization is error-prone, inconsistent across team members, time-consuming for long projects, and subject to human forgetting patterns. At scale, inconsistent context produces inconsistent output. Structured context architecture is systematic where verbalization is ad hoc", C: "Claude can only process context written in Markdown format", D: "Verbalized context consumes too many API tokens" }, correct: "B", explanation: "Manual verbalization doesn't scale: it requires remembering everything every session (unreliable), takes time proportional to project complexity, varies between team members, and creates inconsistency. Chapter 15's thesis: context engineering means systematizing what was previously ad hoc. Structured files are always consistent, complete, and zero-effort to apply — verbalization is always variable, forgettable, and time-consuming." },
        { id: 15, question: "What is the core definition of context engineering that Chapter 15 establishes?", options: { A: "Writing longer and more detailed prompts to get better AI outputs", B: "The deliberate design of what information AI agents receive, in what structure, and at what point in their execution — engineering context as infrastructure to systematically achieve consistent, high-quality AI outcomes rather than relying on ad-hoc prompt improvements", C: "The process of fine-tuning AI models on project-specific data", D: "Configuring API parameters like temperature and max tokens for optimal responses" }, correct: "B", explanation: "Context engineering is systematic infrastructure design: treating AI context — what the agent knows, when it knows it, how it's structured — as an engineering discipline. Not 'write better prompts' but 'design context architecture.' CLAUDE.md, progress files, task systems, mid-stream injection, and context auditing are the tools of context engineering. Chapter 15 frames this as the discipline that separates reliable AI development from unpredictable prompt hoping." },
      ]
    }
  ]
};
