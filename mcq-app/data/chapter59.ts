  ...
  ,
    {
      ...
      mcqs: [
        { id: 1, question: "A startup's business model charges $50/month SaaS subscription. They want to switch to an agent model. What 'Great Inversion' principle determines whether this is economically viable?", options: { A: "Agent models always cost less than SaaS models", B: "The Great Inversion: in SaaS, software is the product (subscription pays for access). In agents, outcomes are the product (payment for results delivered). Viable if: agent-delivered outcome value > (LLM API cost + infrastructure + margin). SaaS revenue doesn't automatically cover agent economics — recalculate from outcome value, not access price", C: "Agent models should be priced at 10x the original SaaS price", D: "The Great Inversion only applies to enterprise customers, not consumers" }, correct: "B", explanation: "Great Inversion economics: SaaS charges for tool access (fixed subscription); agents charge for outcomes delivered (variable per-result). To convert: identify what outcome the agent delivers, determine its value to the customer, calculate full agent cost (LLM API × tokens + infrastructure + development amortization), set price where value >> cost + margin. $50/month SaaS → agent pricing requires knowing 'what outcome does this deliver and what is it worth?'" },
        ...
        { id: 11, question: "What does 'The Great Inversion' mean for how agent application developers should think about pricing?", options: { A: "Agents should always be priced lower than the SaaS tools they replace", B: "Invert from 'how much is the software worth?' to 'how much is the outcome worth?' — price the value created by the agent's actions, not the cost of the underlying compute. An agent that saves a lawyer 10 hours of contract review is worth the value of 10 lawyer-hours, not the API cost of the LLM calls it made", C: "The inversion means API costs should be fully passed through to customers", D: "The Great Inversion refers to inverting the pricing curve (lower price for higher volume)" }, correct: "B", explanation: "Great Inversion pricing philosophy: traditional software pricing anchors on cost (build cost → markup). Agent pricing anchors on outcome value (lawyer saves 10 hours at $400/hour = $4,000 value → price fraction of value). The inversion: from compute-cost-up pricing to outcome-value-down pricing. Chapter 59: if your agent delivers $4,000 of value and costs $8 in API calls, pricing at $400 (10% of value, 50x API cost) is appropriate. Price discovery starts at value, not at cost." },
