...
export const chapter57: Chapter = {
  ...
  lessons: [
    ...
    {
      id: 6,
      title: "Chapter Quiz — MCP Mastery",
      mcqs: [
        { id: 1, question: "A developer says 'my OpenClaw agent is smart but can't actually do anything useful.' The root cause is:", options: { A: "The agent's model is too small for complex tasks", B: "No MCP servers are connected — the agent is confined to its runtime sandbox. Intelligence without reach means it can reason about problems but cannot act on external systems to resolve them", C: "The WhatsApp integration is not configured correctly", D: "The agent needs more skills installed from ClawHub" }, correct: "B", explanation: "Intelligence vs. ability gap: a highly capable reasoning model confined to its sandbox can only chat. MCP servers are the hands that transform reasoning into action — database queries, API calls, record updates. Without them, the agent is a knowledgeable conversationalist, not an operational employee." },
        { id: 2, question: "You need to build an MCP server that queries a PostgreSQL database. The correct first step using the describe-steer-verify workflow is:", options: { A: "Write the full server code from scratch in TypeScript", B: "Install mcp-builder into Claude Code, then describe the requirement in plain English: 'I need an MCP server that connects to a PostgreSQL database and exposes tools for querying customer records and updating order status'", C: "Find a pre-built PostgreSQL MCP server on npm", D: "Write a specification document before touching any code" }, correct: "B", explanation: "Describe-steer-verify starts with description: install mcp-builder into Claude Code, then describe what you need in natural language. The workflow generates the server from your description, which you then steer (refine) and verify (test). Writing from scratch or finding pre-built servers are alternative approaches, not the taught workflow." },
        { id: 3, question: "When should you choose stdio transport over HTTP/SSE for your MCP server?", options: { A: "stdio is always preferred for production deployments", B: "stdio when the MCP server runs locally on the same machine as the OpenClaw agent — it's simpler (no network layer, no port management). HTTP/SSE when the server runs on a different machine or needs to support multiple concurrent connections", C: "stdio is for development only; HTTP/SSE is required for production", D: "The transport choice has no effect on functionality, only naming convention" }, correct: "B", explanation: "Transport selection by topology: same machine = stdio (simpler, no networking), different machines = HTTP/SSE (network protocol required). For a personal OpenClaw deployment with a local PostgreSQL MCP server, stdio is the correct choice. For a remote database server, HTTP/SSE." },
        { id: 4, question: "After building your MCP server with mcp-builder, what does 'steer' mean in the describe-steer-verify workflow?", options: { A: "Deploy the server to a steering test environment", B: "Review the generated specification and code, provide feedback to Claude Code to refine it — 'the query tool needs a date range parameter', 'the update tool should validate the record exists before updating' — iterating until the spec matches requirements", C: "Point the server to the correct database connection string", D: "Run automated tests to verify the server works" }, correct: "B", explanation: "Steer = iterative refinement: after the initial generation, you review what was built and provide specific feedback. Steering corrects gaps, adds missing behaviors, and tightens the specification. It's the iteration phase between first draft and verified implementation — filling the gap between what was described and what was actually needed." },
        { id: 5, question: "What does CLAUDE.md need to be updated with after transport selection?", options: { A: "The database connection string for the MCP server", B: "The MCP server registration — the path to the server binary, the transport type, and any required environment variables. This tells the OpenClaw agent that the new MCP server exists and how to connect to it", C: "The list of tools the MCP server exposes", D: "The WhatsApp test number for end-to-end testing" }, correct: "B", explanation: "CLAUDE.md as server registry: after building an MCP server, it must be registered in CLAUDE.md for the agent to know it exists. The registration includes the server path, transport configuration, and required environment. Without this update, the agent has no awareness of the new tools." },
        { id: 6, question: "What is the value of testing the MCP server from WhatsApp rather than just from the terminal?", options: { A: "WhatsApp testing is required by OpenClaw's deployment process", B: "Terminal testing verifies the server code works; WhatsApp testing verifies the full integration — that the agent correctly decides to use the tool, forms the right parameters, receives the result, and delivers a coherent answer to the user through the channel", C: "WhatsApp testing is faster than terminal testing", D: "Terminal testing cannot connect to production databases" }, correct: "B", explanation: "End-to-end validation: terminal testing is unit testing (does the tool function?); WhatsApp testing is integration testing (does the whole system work?). The agent might have a working MCP server but call it with wrong parameters, misinterpret results, or fail to use it when it should. WhatsApp testing catches all these integration failures." },
        { id: 7, question: "A developer builds an MCP server but the agent never calls it even when the task clearly requires it. The most likely cause is:", options: { A: "The MCP server has a bug that prevents it from running", B: "The tool description is unclear — the agent cannot match the user's request to the tool's purpose if the tool description doesn't clearly describe when and why to use it. Rewrite the tool description to be explicit about its purpose and the types of requests it handles", C: "The agent needs to be retrained to use the new tool", D: "The OpenClaw gateway needs a restart to detect new MCP servers" }, correct: "B", explanation: "Tool description is the routing mechanism: the agent decides which tool to call based on the tool's description in the MCP schema. A vague description ('query data') doesn't tell the agent when to use it. An explicit description ('retrieves customer order history by customer ID — use this when users ask about their orders or order status') gives the agent clear routing signals." },
        { id: 8, question: "What is the correct verify step after steering the MCP server specification?", options: { A: "Ask Claude Code if the implementation looks correct", B: "Test each tool with realistic inputs that match actual production scenarios: correct inputs (verify expected output), edge cases (empty results, missing records), and error cases (invalid inputs, connection failures). Each tool should be tested independently before end-to-end WhatsApp testing", C: "Run the mcp-builder verification script", D: "Deploy to production and monitor for errors" }, correct: "B", explanation: "Systematic verification: test each tool individually (unit tests), covering the happy path (correct input → expected output), edge cases (what happens with no results?), and error handling (what happens when the database is unreachable?). Only after each tool passes individual verification should you proceed to WhatsApp integration testing." },
        { id: 9, question: "How many MCP servers can a single OpenClaw agent connect to?", options: { A: "One MCP server per agent is the architectural limit", B: "Multiple MCP servers can be registered — each providing different external capabilities. A single agent might connect to a PostgreSQL MCP server (customer data), a Gmail MCP server (email), and a custom business system MCP server (inventory) simultaneously", C: "Maximum three MCP servers for performance reasons", D: "Only MCP servers from the official ClawHub registry are supported" }, correct: "B", explanation: "Multi-server architecture: there's no limit to registered MCP servers. Each server exposes a different set of tools from a different external system. The agent's combined toolset is the union of all registered server tools. This is how a single agent gains reach across multiple external systems simultaneously." },
        { id: 10, question: "What makes the describe-steer-verify workflow faster than traditional server development?", options: { A: "The workflow generates perfect code that never needs modification", B: "Plain English description generates a working starting point in minutes rather than hours of coding from scratch. Steering refines iteratively without rewriting — and verification is structured to catch issues early. The human contribution shifts from writing code to reviewing and directing", C: "The workflow eliminates the need for testing", D: "mcp-builder has access to faster servers than developers" }, correct: "B", explanation: "Speed through code generation + iteration: a first draft in minutes (describe) → targeted refinement (steer) → structured testing (verify). Traditional development: hours of writing, then debugging. The workflow compresses the writing phase to minutes and makes the refinement phase explicit and efficient." },
        { id: 11, question: "A customer asks your WhatsApp-connected agent 'what's the current status of order #12345?' Your MCP server is connected to the order database. What happens?", options: { A: "The agent says it cannot access order data", B: "The agent recognizes the order status request, calls the order_status MCP tool with order_id=12345, receives the current status from the database, and replies to the customer with the live order information — real-time data delivered through WhatsApp", C: "The agent asks the user to check the website instead", D: "The agent contacts a human operator who checks the database manually" }, correct: "B", explanation: "MCP in action: this is the architecture's purpose realized. Customer question → agent reasoning → MCP tool call → live database query → result returned → coherent WhatsApp response. From the customer's perspective: instant, accurate, automated order status. No human operator involved, no website redirect." },
        { id: 12, question: "What is the relationship between MCP servers and the Agent Factory thesis invariants?", options: { A: "MCP servers are not related to the Agent Factory thesis", B: "MCP servers implement Invariant 4 (Worker Picks Its Engine — each tool is an engine the worker selects) and Invariant 5 (System of Record — MCP servers provide the interface to the authoritative state store). They are the practical implementation of how Workers reach external systems", C: "MCP servers only relate to Invariant 7 (Nervous System)", D: "MCP servers replace the need for the thesis invariants" }, correct: "B", explanation: "Thesis-MCP mapping: MCP servers are how workers access the System of Record (Invariant 5) through a standardized protocol — each MCP server is an engine the worker selects for a specific external system (Invariant 4). Understanding MCP servers as thesis implementations gives them architectural significance beyond being 'plugins'." },
        { id: 13, question: "What is the security implication of registering an MCP server that has write access to a production database?", options: { A: "No special precautions needed — MCP servers are inherently secure", B: "Write-access tools must be classified as soft-gate or hard-gate in the three-tier security model. A tool that can modify production data should never be auto-executed. Combine MCP capability with appropriate gate classification to prevent unintended database modifications", C: "Write-access MCP servers require NVIDIA NemoClaw for deployment", D: "Write access automatically triggers hard-gate approval without configuration" }, correct: "B", explanation: "MCP + gate classification: an MCP write tool is only as safe as its gate configuration. A write_order_status tool in auto-execute tier means any conversation mentioning orders could trigger a database update. Classify write tools as soft-gate minimum (confirmation required) or hard-gate (explicit WhatsApp approval) based on the business risk of unintended writes." },
        { id: 14, question: "What should a developer document when finishing an MCP server build?", options: { A: "No documentation needed — the code is self-explanatory", B: "Document: each tool's purpose and expected inputs/outputs, the external system it connects to and authentication requirements, the gate tier each tool should be assigned, known limitations and error behaviors, and the test cases used during verification", C: "Only document the installation steps for the MCP server", D: "Documentation is only needed for servers published to ClawHub" }, correct: "B", explanation: "MCP server documentation for maintainability: tool purpose and I/O (for future agents or developers), external system details (for reconfiguration), gate classification guidance (for security configuration), limitations (for runtime expectation management), and test cases (for regression testing after changes). Under-documented MCP servers become unmaintainable." },
        { id: 15, question: "What is the ultimate capability an MCP-enabled OpenClaw agent achieves that a base agent cannot?", options: { A: "The ability to respond to messages faster", B: "Agency in the real world — not just reasoning about what could be done but actually doing it: querying live databases, calling real APIs, updating actual records, sending real emails. The agent becomes a participant in business operations, not just a conversational interface about them", C: "The ability to run longer conversations without context limits", D: "The ability to use multiple AI models for different tasks" }, correct: "B", explanation: "Real-world agency: this is the MCP chapter's central thesis. Base agents reason; MCP-enabled agents act. The distinction is between 'I can tell you about your customer data' and 'I just retrieved your customer data.' MCP transforms the agent from an advisor that describes reality into a participant that changes it." },
      ]
    }
  ]
};
