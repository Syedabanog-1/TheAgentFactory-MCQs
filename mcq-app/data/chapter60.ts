  ...
  ,
    {
      ...
      mcqs: [
        ...
        { id: 3, question: "The '95% Server-Side Update Advantage' means client applications rarely need updates. What architectural principle makes this possible?", options: { A: "All logic runs on the client to avoid server calls", B: "Separating Invariant layers (app structure, authentication, core UI — rarely changes, requires client update) from Variant layers (AI prompts, response formatting, business logic — can change server-side without client update). 95% of product evolution happens in the Variant layer, deployed server-side without requiring users to update their client app", C: "Server-side rendering eliminates the need for client applications", D: "The 95% figure refers to server uptime, not update frequency" }, correct: "B", explanation: "Invariant vs. Variant separation: invariant = app shell that changes infrequently (authentication, navigation, core UI structure) — requires client update when changed. Variant = AI prompts, business logic, response templates — lives on the server and can be updated instantly. If 95% of your product iterations are improving AI behavior (prompt tuning, response formatting, logic changes), those changes deploy server-side without any user update required. Architecture that separates these layers enables rapid iteration without app store approval delays." },
        { id: 4, question: "Your OpenClaw app has user authentication (rarely changes) and AI prompt templates (changes weekly). Which belongs in the Invariant layer and which in the Variant layer?", options: { A: "Both should be in the server — everything is Variant", B: "Authentication → Invariant layer (client app, rarely changes, acceptable to require update when it does). AI prompt templates → Variant layer (server-side, deploy weekly iterations without client updates). Invariant: structural stability. Variant: evolutionary speed", C: "AI prompts → Invariant (stability is critical). Authentication → Variant (can change server-side)", D: "Both should be in the client for offline functionality" }, correct: "B", explanation: "Layer assignment rule: how often does this change and what happens when it changes? Authentication: changes rarely, security-critical — fine as invariant requiring client update when changed. Prompt templates: change weekly, product iteration — must be Variant for rapid deployment. Chapter 60: classify by change frequency and update cost. Slow-changing, client-side = Invariant. Fast-changing, server-updatable = Variant. The 95% advantage only holds if you correctly classify what's variant." },
