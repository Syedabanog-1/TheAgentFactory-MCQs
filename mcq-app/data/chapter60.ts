...
export const chapter60: Chapter = {
  ...
  ,
    {
      id: 7,
      title: "Scenario-Based Mastery — Architecture Decisions & ClawHub Publishing",
      mcqs: [
        { id: 1, question: "Chapter 60 says 'Plans Are Useless' but planning is still valuable. How do you reconcile this apparent contradiction?", options: { A: "Planning should be abandoned entirely in favor of agile development", B: "The plan's value is in the planning process (thinking through requirements, dependencies, trade-offs), not in the plan document itself. Plans become obsolete when encountering real implementation constraints. The six pivots in Chapter 60 show that the plan must evolve with reality — planning builds understanding; the plan document is a starting hypothesis, not a contract", C: "Plans are useful only for small features, not large architectures", D: "The statement is incorrect — plans are always accurate and should be followed exactly" }, correct: "B", explanation: "Plans are useless; planning is invaluable: the act of planning surfaces assumptions, identifies dependencies, and forces consideration of trade-offs. The plan document captures these insights at a point in time. Real implementation reveals the plan's incorrect assumptions. Chapter 60's six pivots demonstrate where plans predictably break: when client needs change, when technical constraints emerge, when dependencies fail. Plans are living documents, not fixed contracts — but you need them to have something to pivot from." },
        { id: 2, question: "What is an Architecture Decision Record (ADR) and when should you create one?", options: { A: "ADR is a code comment explaining why a function exists", B: "ADR is a structured document recording an architectural decision: the context, options considered, decision made, and trade-offs accepted. Create ADRs when making decisions that are: hard to reverse, affect multiple components, or where the reasoning would be unclear to a future developer (or AI agent) reading the code", C: "ADR is a deployment log tracking changes to production infrastructure", D: "ADR is only necessary for decisions made by committee, not individual developers" }, correct: "B", explanation: "ADR value: code shows what was built; ADRs explain why. Future developers (and AI agents reading CLAUDE.md) face the same choices and need context — why JSON files instead of a database? Why this API structure? Without ADRs, teams repeat the same analysis, reach the same conclusions (or wrong ones), and make inconsistent decisions. Chapter 60: write ADRs for decisions where 'future you' (or a new team member) would otherwise ask 'why did they do it this way?'" },
        { id: 3, question: "The '95% Server-Side Update Advantage' means the TutorClaw shim (client component) rarely needs updating. What architectural principle makes this possible?", options: { A: "All logic runs on the client so server updates have no impact", B: "Keeping the shim interface stable — the shim exposes a fixed set of tool contracts to the learner's agent. Server-side improvements (better pedagogy prompts, new curriculum content, pricing changes) deploy without changing the shim interface. Only changes that require new shim capabilities (new tool signatures, new protocols) need a client update", C: "Server-side rendering eliminates the need for client applications entirely", D: "The 95% figure refers to server uptime, not update frequency" }, correct: "B", explanation: "95% server-side advantage mechanism: the TutorClaw shim is a thin client that calls server-side tools. The shim interface (tool names, input/output schemas) changes rarely. What changes frequently — curriculum content, AI guidance quality, pricing rules, pedagogical improvements — lives entirely on the server and deploys without touching the shim. Only structural changes to the tool interface require learners to update the shim. This is why TutorClaw can iterate on content and intelligence weekly while learners update their shim rarely." },
        { id: 4, question: "In TutorClaw's architecture, what specifically are the Invariant layers and Variant layers as Chapter 60 defines them?", options: { A: "Invariant = code that never changes; Variant = code that changes regularly", B: "Invariant layers = what makes TutorClaw what IT IS: PRIMM-AI+ pedagogy, curated content library, pricing model, and learning sequence. Variant layers = HOW TutorClaw is delivered: infrastructure (servers, containers), delivery mechanism (MCP/OpenClaw framework), deployment tools. The six pivots all changed Variant layers while preserving all Invariant layers intact", C: "Invariant = authentication and core UI; Variant = AI prompts and business logic", D: "Invariant = features that users pay for; Variant = free features" }, correct: "B", explanation: "Chapter 60's invariant/variant is about product identity, not update frequency. Invariant layers define what TutorClaw IS — the PRIMM-AI+ instructional approach, curriculum content, pricing model, and learning sequence. If any of these changed, TutorClaw would become a fundamentally different product. Variant layers define how TutorClaw is built and delivered — infrastructure, MCP framework, container approach, deployment tools. All six pivots changed Variant layers (infrastructure, isolation methods, delivery) while the Invariant layers (pedagogy, content, pricing) never changed throughout the project." },
        { id: 5, question: "You're publishing TutorClaw v1.0 to ClawHub. Three weeks later you need to fix a bug in the `generate_guidance` tool. How does versioning affect existing users?", options: { A: "All users must immediately update to the fixed version", B: "Versioning strategy: publish v1.0.1 (patch) with the fix. Users pinned to v1.0 continue working; users who update get the fix. ClawHub versioning allows bug fixes to be deployed without forcing breaking changes on users who depend on v1.0 behavior. Semantic versioning: patch = backwards-compatible bug fix", C: "Bug fixes cannot be deployed without breaking the existing version", D: "All ClawHub users automatically get the latest version with no versioning control" }, correct: "B", explanation: "Semantic versioning for ClawHub: major.minor.patch. Patch (v1.0.0 → v1.0.1): bug fix, backwards compatible, deploy without breaking existing integrations. Minor (v1.0 → v1.1): new features, backwards compatible. Major (v1 → v2): breaking changes requiring consumer migration. Chapter 60's versioning principle: bug fixes should never force breaking changes on users who depended on v1.0 behavior. Proper versioning gives users control over when they adopt changes." },
        { id: 6, question: "The 'Platform Inversion' concept in Chapter 60 describes what shift in how applications are built?", options: { A: "Applications moving from desktop to mobile platforms", B: "AI agents becoming the platform layer that applications are built on top of, rather than applications being tools that call AI APIs. The inversion: traditionally, AI was a feature in apps; post-inversion, apps are functionality delivered through AI agents. OpenClaw/ClawHub apps are built for the agent ecosystem, not for traditional app stores", C: "Cloud platforms replacing on-premise infrastructure", D: "Open source replacing proprietary software platforms" }, correct: "B", explanation: "Platform Inversion: pre-inversion — developers build apps that use AI as a feature (chat widget, recommendation engine). Post-inversion — developers build on the AI agent platform (ClawHub apps, MCP tools, Skills). The distribution channel changes from app stores (user discovers → installs → launches) to agent ecosystems (agent discovers → invokes → returns result). Chapter 60's retrospective: Part 5 of the course prepares developers for post-inversion development — building for AI agents as the primary consumer, not humans as the primary consumer." },
        { id: 7, question: "During TutorClaw development, you encounter a sixth pivot: the JSON persistence approach doesn't work for multi-user scenarios. An ADR recommended JSON at architecture time. What should you do?", options: { A: "Abandon the ADR process — ADRs were wrong and shouldn't be used again", B: "Create a new ADR for the database migration decision — document the pivot: context (JSON hit multi-user limitation), options (PostgreSQL, SQLite, Redis), decision (PostgreSQL for production), trade-offs. ADRs capture the evolution of architectural thinking, not just initial decisions. The JSON ADR was correct for its context; the new ADR captures why that context changed", C: "Simply change the code without documenting the architectural change", D: "ADRs are immutable — the original decision must stand" }, correct: "B", explanation: "ADRs document evolution, not just initial decisions: the original JSON ADR was correct (single-user, simplicity priority). The pivot ADR documents why the context changed (multi-user requirement emerged) and the new decision. Future developers see the reasoning chain: JSON chosen for simplicity → database required for multi-user → PostgreSQL chosen for X reasons. This context prevents future developers from asking 'why not JSON?' and wasting time reconsidering an already-evaluated option." },
        { id: 8, question: "What is the distribution strategy advantage of publishing to ClawHub vs. building a standalone web application?", options: { A: "ClawHub provides free hosting for all applications", B: "ClawHub is an agent-native marketplace — discovery, invocation, and payment infrastructure exists for the agent ecosystem. Standalone web apps require building and marketing their own discovery and distribution. ClawHub positions OpenClaw apps where AI agents can discover and invoke them directly, tapping into the agent ecosystem without rebuilding distribution infrastructure", C: "ClawHub applications are faster than web applications", D: "ClawHub is only for educational applications like TutorClaw" }, correct: "B", explanation: "ClawHub distribution advantage: the marketplace provides agent-native discovery (AI agents can find and invoke apps), payment infrastructure (tier management, Stripe integration), and user trust (ClawHub-vetted apps). Building standalone: requires building all of this from scratch. Chapter 60: ClawHub is the distribution layer for the agent ecosystem — publishing to ClawHub is similar to publishing to an app store, but for the AI agent consumer ecosystem rather than human consumers." },
        { id: 9, question: "Chapter 60's 'Part 5 Retrospective' synthesizes lessons from building OpenClaw apps. What is the most important meta-lesson about building AI agent applications?", options: { A: "AI agent applications are too complex for individual developers", B: "The technical skills (MCP, JSON persistence, monetization, versioning) are learnable — the hard part is the architectural judgment about what to build into the Invariant layer vs. the Variant layer, when to create ADRs, and how to version for evolution without breaking consumers. Good judgment about these decisions determines whether the application can evolve without accumulating technical debt", C: "Always use the latest AI model for best results", D: "ClawHub is the only valid platform for AI agent applications" }, correct: "B", explanation: "Meta-lesson from Part 5 retrospective: technical skills are the table stakes — MCP, JSON, Stripe, webhooks are learnable. Architectural judgment is the differentiator: invariant/variant separation (enables rapid iteration), ADRs (prevents repeated analysis, guides future evolution), versioning strategy (enables ecosystem growth without breaking consumers). These decisions compound — bad early architecture decisions constrain every future evolution. Good early decisions create a foundation that enables rapid, confident iteration." },
        { id: 10, question: "What is the relationship between Chapter 58 (building TutorClaw) and Chapter 60 (architecture decisions)? How do they work together?", options: { A: "They are independent chapters covering unrelated topics", B: "Chapter 58 provides the implementation story (build decisions made under uncertainty); Chapter 60 provides the architectural framework (ADRs, versioning, invariant/variant) for making those decisions systematically. Together: Ch58 shows what was built and why decisions were made; Ch60 shows how to make similar decisions in a repeatable, documented, evolvable way", C: "Chapter 60 overrides all architectural decisions made in Chapter 58", D: "Chapter 58 is for beginners; Chapter 60 is for advanced developers only" }, correct: "B", explanation: "Chapter 58 + 60 relationship: Chapter 58 is narrative (TutorClaw build story — here's what we built and the decisions we made). Chapter 60 is framework (here's how to make these decisions systematically — ADRs, invariant/variant, versioning). The TutorClaw experience informs the framework: 'we should have documented this JSON decision in an ADR' (Chapter 60) because 'the JSON→database pivot would have been cleaner with documented context' (Chapter 58 lesson). Experience informs framework; framework guides future practice." },
        { id: 11, question: "When does the '95% server-side update' claim break down? What scenario requires a client-side update even for a Variant-layer change?", options: { A: "It never breaks down — all changes can always be server-side", B: "When a Variant-layer change requires a new capability that doesn't exist in the current client version — e.g., a new response format that the old client doesn't know how to render. Server-side content changes work without client updates only when the client already knows how to handle the new content format", C: "When more than 50% of users are on mobile devices", D: "Server-side updates always require client updates for security reasons" }, correct: "B", explanation: "95% advantage caveat: the server-side update only works if the client can handle the new server output. New prompt templates (same response format, better content) → pure server-side. New response type the old client doesn't render → requires client update to handle new format. The Variant layer is 'content and logic that clients already know how to consume.' Changes that require new client capabilities break the server-side-only update model. Chapter 60: architecture for the 95% advantage requires designing client capabilities to be general enough to handle variant content evolution." },
        { id: 12, question: "What problem do ADRs solve for AI agents that work with your codebase?", options: { A: "ADRs are only useful for human team members, not AI agents", B: "ADRs in CLAUDE.md or referenced from it give AI agents the architectural reasoning behind decisions. Without ADRs, AI agents see code patterns but not why they exist — potentially 'fixing' a constraint that was intentional (e.g., changing JSON to a database, unaware it was chosen for offline-first reasons). ADRs prevent AI from accidentally undoing intentional architectural decisions", C: "ADRs slow down AI agents by adding too much context", D: "AI agents automatically infer architectural reasoning from code patterns" }, correct: "B", explanation: "ADRs as AI agent context: AI agents reading codebases see what was built, not why constraints exist. An ADR saying 'JSON chosen for offline-first requirements — do not add database dependency' prevents an AI agent from helpfully 'improving' the code by adding a database (which breaks offline use). Chapter 60: ADRs are double-duty documentation — for human developers AND for AI agents that will maintain the code. Document the why to prevent both humans and AI from undoing intentional choices." },
        { id: 13, question: "You're building a ClawHub app that will be used by other AI agents as a tool. How does this change the 'distribution strategy' compared to building for human users?", options: { A: "No difference — the same distribution strategy works for both audiences", B: "Agent-consumer distribution: schema precision (agents need exact input/output contracts, not approximate descriptions), discovery via tool manifests (agents find tools by capability metadata, not marketing copy), reliability over UX (agents retry, don't forgive inconsistency) — all different from human-user distribution where UI and onboarding experience drive adoption", C: "Building for AI agents is easier because they don't care about design", D: "Agent consumers require in-person demonstrations before adoption" }, correct: "B", explanation: "Agent vs. human consumer differences: humans browse (discovery via search/recommendation/social). Agents search manifests (discovery via capability metadata matching). Humans tolerate approximate interfaces. Agents require exact schemas. Humans forgive inconsistency. Agents propagate errors and retry on failures. Chapter 60's distribution strategy for ClawHub: treat the tool manifest as the marketing copy AND the API contract — both must be precise. Agent-native distribution is fundamentally different from human-native distribution." },
        { id: 14, question: "Six pivots occurred during the OpenClaw Part 5 project. What does the occurrence of six pivots reveal about the nature of software architecture decisions?", options: { A: "Six pivots indicate poor initial planning — better planning would have prevented all pivots", B: "Pivots are a natural feature of building in novel domains (AI agent applications). Initial plans encode the best understanding available at planning time; implementation reveals constraints, user needs, and technical realities that couldn't be anticipated. Six pivots in one project reflects the novelty of the domain, not planning failure. ADRs document each pivot for future reference", C: "Six pivots indicate the architecture was fundamentally wrong from the start", D: "Pivots only occur when developers lack sufficient technical skills" }, correct: "B", explanation: "Pivots as learning, not failure: in novel domains (building AI agent apps when the patterns are still being discovered), plans based on incomplete knowledge will encounter realities that require adaptation. Six pivots ≠ bad planning; it means building something new where each implementation step reveals something the plan didn't know. Chapter 60's framing: 'Plans Are Useless' acknowledges this reality. The goal isn't to predict all pivots upfront — it's to adapt quickly when pivots arrive, document decisions (ADRs), and preserve the reasoning for future iterations." },
        { id: 15, question: "What is the synthesis of Chapters 58-60 for a developer building their first OpenClaw application for ClawHub?", options: { A: "Start with monetization first — economics determine everything", B: "Blueprint tools before building (Ch58) → Build in dependency order with tests (Ch58) → Document architectural decisions via ADRs (Ch60) → Design Invariant/Variant layers deliberately (Ch60) → Version from the start (Ch60) → Publish with a complete manifest (Ch58/60). The three chapters together form the complete methodology for building production-quality ClawHub applications", C: "Choose the cheapest architecture and iterate", D: "Start with the UI and work backwards to the architecture" }, correct: "B", explanation: "Chapters 58-60 synthesis: Ch58 provides the implementation methodology (blueprint → dependency-ordered build → integration test → publish). Ch60 provides the architectural decision framework (ADRs, invariant/variant, versioning). Together: before coding → blueprint and ADR key decisions; during coding → dependency order, test integration; architecture → classify invariant/variant; publishing → complete manifest; evolution → versioning strategy. These three chapters operationalize the complete lifecycle from 'idea for a ClawHub app' to 'published, evolvable, production application.'" },
      ]
    }
  ]
};
