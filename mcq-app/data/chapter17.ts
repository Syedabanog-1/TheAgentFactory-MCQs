...
export const chapter17: Chapter = {
  ...
  ,
    {
      id: 11,
      title: "Scenario-Based Mastery — Seven Principles of Agent Problem Solving",
      mcqs: [
        { id: 1, question: "An AI agent needs to determine which version of Python is installed, check whether a package exists, and verify a config file. What Principle 1 approach handles all three efficiently?", options: { A: "Ask the user to manually check and report back", B: "Bash is the key — bash commands (`python --version`, `pip show package`, `cat config.json`) answer each question directly and immediately. The agent executes shell commands to gather facts, not asking users to perform lookups", C: "Use only built-in Python introspection functions", D: "Request file access permissions from the operating system" }, correct: "B", explanation: "Principle 1 (Bash is the Key): bash commands are the universal information-gathering tool. `python --version` → version. `pip show flask` → package existence. `cat config.json` → config contents. The agent doesn't ask, guess, or wait — it runs commands and reads results. This principle transforms the agent from dependent on user-provided facts to independently verifiable through direct system interaction." },
        { id: 2, question: "You need to transform customer data from CSV format for a database insert, but no built-in transformation tool exists. What Principle 2 approach solves this?", options: { A: "Request a new tool to be built by the development team", B: "Code as Universal Interface — write a short script to perform the transformation. Code can do anything computationally expressible; when no built-in tool exists, generate the tool as code. The agent writes a Python script that reads the CSV, transforms, and outputs insert statements", C: "Use a database migration tool that may not support the exact format", D: "Manually describe the transformation for a human to execute" }, correct: "B", explanation: "Principle 2 (Code as Universal Interface): code is the escape hatch for gaps in built-in capabilities. Need a format converter? Write one. Need a custom parser? Code it. The agent generates purpose-specific scripts rather than being limited to pre-built tools. This principle means the agent's capability space equals 'everything computationally expressible' — not just what pre-built tools provide." },
        { id: 3, question: "An AI agent refactors a function and claims it works correctly. What Principle 3 step must always follow implementation?", options: { A: "Commit the code immediately to preserve the working state", B: "Verification as core step — run tests, execute the code path, check outputs against expected behavior. Claims about correctness are hypotheses; verification produces evidence. Never accept 'it should work' — verify that it does work through execution", C: "Have a human developer manually review the code", D: "Document the changes in the commit message" }, correct: "B", explanation: "Principle 3 (Verification): implementation without verification is a claim, not a delivery. After refactoring: run the test suite, execute the refactored function with known inputs, compare outputs to expected values. The principle: verification closes the gap between 'code was written' and 'feature works.' Chapter 17: treat every AI-generated implementation as a hypothesis to be tested, not a fact to be trusted." },
        { id: 4, question: "An agent is building a complex authentication system. Instead of implementing the full system at once, it builds and tests login, then logout, then token refresh separately. What principle is being applied?", options: { A: "Waterfall development methodology", B: "Principle 4 (Small, Reversible Decomposition) — decompose complex work into small, independently verifiable pieces. Each piece can be tested and rolled back if it fails, without losing the entire implementation. Small steps beat big leaps for reliability", C: "Test-Driven Development methodology", D: "The agent is being inefficient — it should implement the full system at once" }, correct: "B", explanation: "Principle 4 (Small, Reversible Decomposition): complex work decomposed into small steps where each step is: (a) independently testable, (b) reversible if wrong, (c) committed only when verified. Login first → verify → logout → verify → token refresh → verify. If token refresh fails, only that piece is reverted — login and logout remain intact. Chapter 17: small reversible steps is the risk management principle for agent-driven development." },
        { id: 5, question: "An agent processes 10,000 records in a batch job. Midway through, the session crashes and restarts. How does Principle 5 prevent reprocessing from the beginning?", options: { A: "The agent has to reprocess all 10,000 records — there's no recovery mechanism", B: "Principle 5 (Persisting State in Files) — write progress to a state file after each batch. On restart, read the state file to determine resume point. Persisted state survives session crashes; in-memory state does not. The agent resumes from the last checkpoint, not from zero", C: "Use a database transaction to track progress", D: "Process all records simultaneously to avoid the crash scenario" }, correct: "B", explanation: "Principle 5 (Persisting State): in-memory state dies with the session. File-based state survives. Batch job pattern: after processing records 1-1000, write '{progress: 1000, status: running}' to state file. On crash/restart, read state file → resume from record 1001. This principle applies to any long-running work: checkpoints in files enable reliable resumption. Chapter 17: state belongs in files, not in session memory, for any work that can be interrupted." },
        { id: 6, question: "An agent is about to execute `rm -rf /app/data` to clean up old files. What Principle 6 behavior should precede this action?", options: { A: "Execute immediately — the agent's judgment about cleanup is sufficient", B: "Principle 6 (Constraints and Safety) — pause before destructive, irreversible actions. Verify: Is /app/data the correct path? Are these truly old files? Is there a backup? Propose the action for confirmation before executing. Irreversible actions require explicit verification, not automatic execution", C: "Log the action but execute it immediately", D: "Only humans should perform file deletions — AI agents should never execute rm commands" }, correct: "B", explanation: "Principle 6 (Constraints and Safety): irreversible actions demand explicit confirmation. `rm -rf` cannot be undone; misidentifying the path means permanent data loss. Safety behaviors: (1) verify the path resolves to the intended target, (2) check for unintended consequences (is /app/data used by a running service?), (3) propose the action and wait for confirmation before executing. Chapter 17: agents must apply higher scrutiny to irreversible actions — the cost of 'wait and confirm' is low; the cost of 'execute and be wrong' is high." },
        { id: 7, question: "An agent completes a deployment task. You have no visibility into what it did — no logs, no audit trail, no way to verify what happened. What principle was violated?", options: { A: "Principle 1 — should have used bash commands", B: "Principle 7 (Observability) — agents must produce visible evidence of their actions: logs, output files, status messages, or audit trails. Without observability, failures are invisible, debugging is impossible, and trust in agent actions cannot be established", C: "Principle 3 — verification was skipped", D: "Principle 4 — the task wasn't decomposed enough" }, correct: "B", explanation: "Principle 7 (Observability): agent actions must produce auditable evidence. Deployment completed — but what was deployed, to what environment, at what time, with what result? Without logs: failures appear as silence, not errors; debugging requires re-running the action; trust requires blind faith. Observability principle: write action logs, output status messages, create audit files. Human oversight of agents requires visible evidence of what agents did." },
        { id: 8, question: "What is the relationship between Principle 3 (Verification) and Principle 7 (Observability)?", options: { A: "They are unrelated principles that apply in different contexts", B: "They are complementary: Verification requires Observability — you can only verify what you can observe. If an agent produces no visible output, there's nothing to verify. Observability makes verification possible; Verification uses the observability artifacts to confirm correctness", C: "Observability replaces the need for Verification", D: "Verification makes Observability unnecessary — if it's verified, you don't need logs" }, correct: "B", explanation: "Verification depends on Observability: to verify a deployment succeeded (Principle 3), you need visible evidence (Principle 7) — deployment logs, health check responses, error rates in monitoring. Without observability, verification is guesswork. The principles form a system: Principle 7 produces evidence → Principle 3 uses evidence to confirm correctness. An agent that verifies but doesn't persist observations leaves no audit trail; an agent that observes but doesn't verify misses correctness failures." },
        { id: 9, question: "An agent is given a task: 'optimize the database.' Without further constraints, it drops all indexes, rewrites queries, and restructures tables. The database is unusable. What principle failure occurred?", options: { A: "The agent should have asked for clarification before starting", B: "Principle 6 (Constraints and Safety) failure — 'optimize' is ambiguous and the agent executed destructive changes without explicit scope constraints or confirmation. Constraints principle: define safe operating boundaries before execution. Ambiguous instructions + destructive capability = dangerous autonomy", C: "Principle 4 failure — the task wasn't decomposed", D: "Principle 1 failure — bash was used incorrectly" }, correct: "B", explanation: "Principle 6 failure mode: ambiguous instructions + broad capability = unintended destruction. 'Optimize' could mean add indexes (additive) or restructure tables (destructive). Without constraints, the agent interpreted maximally — doing everything possible to optimize. Safety principle: before executing potentially destructive work, define constraints: what can be changed, what cannot, what requires confirmation. Agents need explicit boundaries, not just goals." },
        { id: 10, question: "What operational best practice does Chapter 17 recommend for handling long-running agent tasks that may span hours?", options: { A: "Monitor the agent constantly to intervene when needed", B: "Combine Principle 5 (Persisting State) with Principle 7 (Observability): write progress checkpoints to files at regular intervals and produce status logs. This allows resumption on failure and provides visibility into progress without requiring constant monitoring", C: "Break all long-running tasks into subtasks under 5 minutes each", D: "Use a more powerful AI model that can complete tasks faster" }, correct: "B", explanation: "Long-running task best practice: checkpoint + observe. Principle 5: write state files at intervals (process batch 1-100, write checkpoint, batch 101-200, write checkpoint). Principle 7: produce progress logs (processed 1000/10000 records, 10% complete). Combined: if the task fails at hour 3, the checkpoint enables resumption from the last checkpoint, and logs show exactly where and why it failed. Chapter 17: reliability for long-running work requires both state persistence and observability." },
        { id: 11, question: "Principle 4 says 'small, reversible decomposition.' What makes a step 'reversible'?", options: { A: "A step is reversible if it takes less than 1 minute to execute", B: "A step is reversible if its effects can be undone without loss — code changes can be reverted (git), file creates can be deleted, database inserts can be rolled back. Irreversible steps (dropping tables, deleting files, sending emails) require extra caution before execution", C: "All steps are reversible if you have backups", D: "Reversibility is determined by whether the step was documented" }, correct: "B", explanation: "Reversibility in practice: adding code (reversible via git revert), creating files (reversible via delete), inserting records (reversible via delete/rollback). Dropping a production table (irreversible — data gone), sending a customer email (irreversible — can't unsend), billing a credit card (irreversible — requires refund process). Principle 4: identify which steps are reversible and which aren't. For irreversible steps, apply Principle 6 safety checks. For reversible steps, proceed with confidence." },
        { id: 12, question: "An agent uses Python script to analyze 50 log files, producing a summary report. This is an example of which principle?", options: { A: "Principle 1 — using bash to read files", B: "Principle 2 (Code as Universal Interface) — writing purpose-specific code to fill a capability gap. No built-in log analysis tool exists, so the agent generates a Python script tailored to the specific log format and analysis requirements. Code extends the agent's capabilities beyond pre-built tools", C: "Principle 5 — persisting analysis results to files", D: "Principle 7 — producing observable output from the analysis" }, correct: "B", explanation: "Principle 2 example: 50 log files with a specific format, custom analysis requirements → no pre-built tool fits exactly. Solution: write a Python script that reads the format, applies the analysis logic, and produces the required output. The agent's capability isn't limited to available tools — it generates tools. This is the power of Principle 2: code enables arbitrary computation, making the agent's capability space essentially unlimited for well-specified tasks." },
        { id: 13, question: "A team deploying AI agents for production work notices that agents sometimes make changes they cannot explain or verify. Which combination of principles does this violate?", options: { A: "Only Principle 3 is violated", B: "Both Principle 3 (Verification) and Principle 7 (Observability) — unexplainable changes mean no verification was performed (Principle 3) and no observable audit trail was produced (Principle 7). Production AI agents must verify their actions produce correct outcomes AND leave observable evidence of what was done", C: "This is normal AI behavior that cannot be prevented", D: "Only Principle 6 is violated" }, correct: "B", explanation: "Production reliability requires Principles 3 and 7 together: Principle 3 (Verify) — after each change, confirm it produced the intended outcome (run tests, check outputs). Principle 7 (Observe) — produce an audit trail showing what was changed, when, and with what result. Both together: verified actions that leave evidence. 'Unexplainable changes' = neither principle applied. Chapter 17's operational best practices: production agents must be verifiable (Principle 3) and auditable (Principle 7)." },
        { id: 14, question: "What is the core insight of 'Putting It All Together' in Chapter 17 — how do the seven principles form a coherent system?", options: { A: "The seven principles are independent tips that can be applied in any order", B: "The principles form a problem-solving loop: Bash gathers facts (P1) → Code fills gaps (P2) → Verification confirms correctness (P3) → Small steps manage risk (P4) → State persistence enables continuity (P5) → Safety constraints prevent harm (P6) → Observability enables oversight (P7). Each principle addresses a specific failure mode in autonomous agent execution", C: "Only some principles apply to any given task — select based on context", D: "The principles are a curriculum, not operational guidelines" }, correct: "B", explanation: "Seven principles as a coherent system: P1+P2 (capability) → P3+P7 (reliability) → P4+P5 (continuity) → P6 (safety). Each principle addresses where agents fail: P1 prevents information blindness, P2 prevents capability gaps, P3 prevents shipping defects, P4 prevents irreversible mistakes, P5 prevents progress loss, P6 prevents unintended harm, P7 enables trust. Together they define a complete framework for reliable autonomous agent operation." },
        { id: 15, question: "A developer argues 'These seven principles add overhead — just tell the AI to do the task.' When is this argument correct and when is it wrong?", options: { A: "The argument is always correct — principles are unnecessary overhead", B: "Correct for trivial, low-risk, easily reversible tasks with immediate verification. Wrong for complex, multi-step, long-running, or potentially destructive tasks where the cost of agent failure exceeds the overhead of applying principles. Principles are risk management — their ROI scales with task risk", C: "The argument is always wrong — principles must always be applied", D: "The argument is only relevant for production systems, not development environments" }, correct: "B", explanation: "Principles as risk management: 'rename this variable' requires none of the seven principles (trivial, reversible, immediately verifiable). 'Migrate production database' requires all seven (complex, irreversible, multi-step, hours-long, potentially destructive). Chapter 17's operational guidance: match principle application to risk level. Low risk, simple tasks → minimal overhead. High risk, complex tasks → full principle application. Engineering judgment determines which principles apply — they're a toolkit, not a religion." },
      ]
    }
  ]
};
