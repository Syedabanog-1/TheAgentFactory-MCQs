...
export const chapter16: Chapter = {
  ...
  ,
    {
      id: 11,
      title: "Scenario-Based Mastery — Spec-Driven Development",
      mcqs: [
        { id: 1, question: "A developer jumps straight into coding a new feature based on a verbal description from the product manager. Two weeks later, the implementation doesn't match what was wanted. What SDD practice would have prevented this?", options: { A: "Using a better code editor with AI assistance", B: "Writing a specification before coding — SDD's core principle: spec defines what to build (verifiable requirements), code implements it. The gap between verbal intent and implementation is closed by written, reviewable specifications reviewed before a line of code is written", C: "Asking for more detailed verbal requirements", D: "Building a prototype first to validate understanding" }, correct: "B", explanation: "SDD prevents implementation drift: verbal requirements are ambiguous and unverifiable. A written spec forces explicit commitment to behavior, edge cases, and constraints before implementation begins. When the PM reviews the spec before coding, misunderstandings surface at zero implementation cost. Chapter 16: 'Specs beat vibe coding because they make intent explicit, reviewable, and verifiable — creating a contract between what was wanted and what gets built.'" },
        { id: 2, question: "What are the Three Levels of SDD and what distinguishes them?", options: { A: "Design, Development, Deployment — the three phases of software delivery", B: "Spec-First (write spec before any code, spec guides then is discarded), Spec-Anchored (both spec and code are maintained together as living artifacts), and Spec-as-Source (spec is the primary artifact; code is regenerated from it on demand) — three levels of commitment to the specification", C: "Requirements, Architecture, and Testing — three categories of documentation", D: "Junior, Mid, and Senior — three skill levels for developers using SDD" }, correct: "B", explanation: "The three SDD levels form a spectrum of specification commitment: Spec-First = write the spec, Claude implements it, spec is then discarded (best for single-session tasks and clear requirements). Spec-Anchored = both spec and code are maintained as living artifacts (team standard for multi-contributor projects). Spec-as-Source = the spec is the primary artifact and code is regenerated from it on demand (experimental — faces determinism challenges in practice). Each level suits different project scales and requirements clarity." },
        { id: 3, question: "Your CLAUDE.md file contains project conventions, but your AI agent consistently ignores specific coding standards like 'always use TypeScript strict mode.' What SDD concept explains this and how do you fix it?", options: { A: "The AI model doesn't support TypeScript", B: "CLAUDE.md as Project Constitution: if the standard isn't in CLAUDE.md (or is buried and unclear), the agent doesn't treat it as a project-wide requirement. Fix: add 'STRICT REQUIREMENT: TypeScript strict mode always enabled' as a prominent section in CLAUDE.md", C: "TypeScript strict mode must be enforced through eslint, not CLAUDE.md", D: "AI agents cannot enforce programming language configuration" }, correct: "B", explanation: "CLAUDE.md is the Project Constitution — it defines project law. If a standard isn't in CLAUDE.md or is not clearly stated as a requirement, agents treat it as optional. SDD practice: make critical standards explicit, prominent, and unambiguous in CLAUDE.md. 'STRICT REQUIREMENT' framing signals non-negotiable constraints that override agent defaults. The Project Constitution's authority comes from being explicit, not implicit." },
        { id: 4, question: "In Phase 1 of the Four-Phase SDD Workflow, you conduct 'Parallel Research' before writing the specification. What does this phase accomplish?", options: { A: "Parallel Research means multiple developers writing specs simultaneously", B: "Gathering information on competing approaches, existing solutions, technical constraints, and domain patterns before committing to a specification — informed specs avoid reinventing wheels and surface technical risks before they become implementation costs", C: "Running the AI agent on multiple tasks in parallel during research", D: "Researching which AI model is best for the development task" }, correct: "B", explanation: "Parallel Research (Phase 1): before specifying, understand the solution space. Research existing libraries, competitor implementations, architectural patterns, and technical constraints. This prevents specifying an approach that ignores better-known solutions. An SDD spec written without research may mandate reinventing what already exists. Chapter 16's parallel research phase ensures specs are informed by domain knowledge, not wishful thinking." },
        { id: 5, question: "You've written a technical spec. Before implementing, you conduct a 'Refinement Interview' with the AI. What is the purpose of this interview?", options: { A: "Testing whether the AI model is intelligent enough to understand the spec", B: "Asking the AI to identify ambiguities, missing cases, inconsistencies, and implementation risks in the spec — using AI as a spec reviewer to surface issues before implementation begins. Better to find spec gaps in conversation than in broken code", C: "Teaching the AI about the domain through a Q&A session", D: "Getting the AI to estimate implementation timeline" }, correct: "B", explanation: "Refinement Interview: the AI reviews the spec as a critical analyst, not an implementer. Ask: 'What's ambiguous in this spec? What cases aren't covered? What could go wrong with this approach?' AI often surfaces issues human authors miss (they're too close to the spec). Finding a spec gap in conversation takes minutes; finding it in failed implementation takes hours. Chapter 16: refinement interviews are spec QA before the implementation clock starts." },
        { id: 6, question: "A developer says 'I don't need specs for small features — they're quick to build.' When does Chapter 16's Decision Framework say SDD is NOT the right approach?", options: { A: "SDD should always be used regardless of feature size", B: "SDD has overhead — for trivially small, well-understood changes (renaming a variable, fixing a typo, adding a log statement), the spec-writing cost exceeds the benefit. SDD ROI is positive for features with ambiguity, multiple components, team dependencies, or unfamiliar domains", C: "SDD should never be used for features that take less than a week", D: "SDD is not applicable to frontend development" }, correct: "B", explanation: "SDD Decision Framework: SDD overhead (research, spec writing, review) must be justified by the risk of building wrong. For trivially clear, low-risk changes, SDD overhead exceeds benefit. SDD is high-ROI for: features with unclear requirements, multi-component implementations, team dependencies, unfamiliar domains, or anything where 'build wrong' means days of rework. Apply engineering judgment — match process overhead to risk level." },
        { id: 7, question: "Phase 4 of the SDD workflow is 'Task-Based Implementation.' How does this phase use the specification?", options: { A: "The spec is discarded after Phase 3 — Phase 4 is pure coding", B: "The spec becomes a checklist: AI implements each specified requirement as a verifiable task, checking completion against the spec. Implementation is driven by the specification, and completion is verified against it — not 'does the code exist?' but 'does it satisfy the spec?'", C: "Phase 4 involves rewriting the spec as code comments", D: "The AI agent creates its own implementation plan independent of the spec" }, correct: "B", explanation: "Task-Based Implementation uses spec as a contract: each spec item becomes a task the AI implements and verifies. Implementation is complete when ALL spec requirements are satisfied, not when 'the code runs.' This prevents the common failure where 'done' means 'coded something' rather than 'satisfied the requirements.' Chapter 16's Phase 4: the specification is the definition of done — verify against it, not against the developer's impression of what was wanted." },
        { id: 8, question: "What is 'Vibe Coding' and what specific problem does SDD solve that vibe coding creates?", options: { A: "Vibe coding is a valid methodology for rapid prototyping", B: "Vibe coding is ad-hoc development driven by immediate intuition — describe roughly what you want and implement whatever AI generates. Problem: no verifiable definition of done, no constraint documentation, no specification to validate against. SDD replaces 'build something close' with 'build exactly what was specified'", C: "Vibe coding is a style of coding that prioritizes developer wellbeing", D: "Vibe coding is the same as agile development" }, correct: "B", explanation: "Vibe coding vs. SDD: vibe coding is 'describe, generate, ship without verification.' It produces working demos that fail in production because edge cases, constraints, and requirements weren't specified. SDD replaces vibes with contracts: explicit requirements, reviewable specs, verification against spec. The problem vibe coding creates: you can't verify completion (no spec = no done criteria). SDD's solution: specifications ARE the definition of done." },
        { id: 9, question: "You're writing a specification for a payment system. A colleague suggests skipping the constraint section because 'we can figure that out during coding.' What SDD principle argues against this?", options: { A: "Constraints are optional in specifications — they can be added later", B: "Constraints are the most expensive items to add during coding — they change architecture and require rework. SDD specifications must include security requirements, data limits, error handling, and compliance constraints upfront because retrofitting constraints costs 10-100x what specifying them upfront costs", C: "Constraints are defined by the operations team, not developers", D: "Payment systems are exempt from constraint specifications due to complexity" }, correct: "B", explanation: "Constraints discovered during coding cause expensive rework: a payment spec without rate limiting discovered during QA means redesigning the API layer. A spec with explicit constraints allows the implementation to be designed around them from the start. SDD principle: specify constraints before implementation — security, data limits, error handling, compliance. The cost curve is steep: constraints in spec = design-time decision; constraints in production = emergency rework." },
        { id: 10, question: "How does SDD change the role of AI agents in development from a 'guesser' to an 'implementer'?", options: { A: "AI agents cannot use specifications — they work only from natural language descriptions", B: "Without specs, AI guesses at requirements from ambiguous descriptions. With specs, AI implements against explicit requirements — the spec is a contract the AI executes, not an interpretation challenge. Specs convert 'what did they mean?' into 'what does the spec require?'", C: "SDD eliminates the need for AI agents in development", D: "AI agents are better at guessing than following specifications" }, correct: "B", explanation: "SDD transforms AI from interpreter to implementer: vague descriptions require AI to infer, guess, and fill gaps — producing outputs that satisfy the description but miss intent. Specifications provide unambiguous requirements that AI executes faithfully. Chapter 16's insight: the difference between AI that produces 80% correct code and 99% correct code is specification quality, not model intelligence. Specs make AI reliable because reliability requires clear contracts." },
        { id: 11, question: "A team is building a multi-tenant SaaS platform. They have stakeholder requirements but haven't started coding. Which SDD phase order is correct?", options: { A: "Code first, then document what was built", B: "Phase 1 (Parallel Research) → Phase 2 (Write Specification) → Phase 3 (Refinement Interview) → Phase 4 (Task-Based Implementation) — research informs the spec, refinement improves it, then implementation executes against it", C: "Phase 2 (Write Specification) → Phase 1 (Parallel Research) → Phase 4 (Implementation) — skip refinement for speed", D: "All phases can be run simultaneously to save time" }, correct: "B", explanation: "Four-phase order is sequential for a reason: research (understanding options) must precede specification writing (committing to an approach). Writing specs before research creates uninformed specs. Refinement reviews the spec for gaps before any code is written. Implementation executes the refined spec. The order prevents the most common failure modes: uninformed specs (fixed by Phase 1), ambiguous specs (fixed by Phase 3), and unspecified implementations (fixed by Phase 4 task-tracking)." },
        { id: 12, question: "A team is prototyping a new feature but requirements are unclear. They expect to discover the real requirements through building. Then they'll solidify the design for production. Which SDD level should they use and why?", options: { A: "Spec-First — always write the spec first regardless of requirement clarity", B: "Spec-Anchored — allows exploration first, then anchors the discovered requirements in a spec before production. Requirements that can only be found through building are exactly what Spec-Anchored is designed for: explore freely, then spec what worked before committing to production code", C: "Spec-as-Source — regenerate code from specs throughout exploration", D: "Skip SDD entirely for prototypes" }, correct: "B", explanation: "Spec-Anchored for discovery: 'I don't know exactly what I need to build until I've tried something' is the trigger for Spec-Anchored. Spec-First requires knowing requirements before writing the spec — impossible for novel problems. Spec-Anchored legitimizes exploration as a phase: discover through vibe coding, capture what works in a spec, then implement reliably with SDD for production. Both rigor and exploration, in the right sequence." },
        { id: 13, question: "A specification says 'the API should be fast.' An AI agent builds an API. Is this specification complete? What SDD writing principle applies?", options: { A: "Yes — 'fast' is sufficient requirement for most systems", B: "No — specifications must be verifiable. 'Fast' is unmeasurable. Correct: 'API must respond within 200ms at p95 for payloads up to 1MB.' Effective specs use quantified, testable criteria — not subjective adjectives that both parties interpret differently", C: "The AI agent will define 'fast' based on industry standards", D: "Specifications should use qualitative language to leave implementation flexibility" }, correct: "B", explanation: "SDD specification quality: requirements must be verifiable, not aspirational. 'Fast,' 'secure,' 'scalable' are adjectives, not requirements. Effective specs quantify: 'p95 latency < 200ms,' 'AES-256 encryption for data at rest,' 'supports 10,000 concurrent connections.' The test: can you write a test that definitively passes or fails against this requirement? If not, the requirement isn't complete. Chapter 16: ambiguous specs produce ambiguous implementations." },
        { id: 14, question: "You're writing specifications for an AI agent that will perform customer support. What makes this specification higher priority for SDD vs. a simple utility script?", options: { A: "Customer support is easier to specify than utility scripts", B: "Customer support has: multiple stakeholders (product, legal, support teams), compliance constraints (privacy regulations), complex edge cases (escalation logic, emotional customer handling), reputation risk if wrong, and integration dependencies (CRM, ticketing). High complexity + high risk = high SDD ROI", C: "SDD is equally important for all software regardless of risk", D: "AI-powered customer support is exempt from SDD because AI is flexible" }, correct: "B", explanation: "SDD ROI scales with risk and complexity: customer support AI touches compliance (PII handling), reputation (customer interactions), multiple systems (CRM, ticketing), and complex logic (escalation trees). Getting it wrong has measurable business consequences. A utility script renaming files has low risk and clear behavior — minimal SDD overhead needed. Customer support AI has high risk and complex requirements — maximum SDD investment justified. Chapter 16's decision framework: match spec rigor to risk level." },
        { id: 15, question: "How does the SDD approach address the challenge of AI agents producing inconsistent quality across different team members' requests?", options: { A: "SDD standardizes model parameters so all team members get the same output", B: "SDD standardizes the input: when all team members work from the same reviewed specification and Project Constitution (CLAUDE.md), they provide the same quality context to AI — consistent input produces consistent output. Without specs, each member's ad-hoc descriptions produce inconsistent AI behavior", C: "SDD restricts which team members can use AI agents", D: "SDD requires all team members to memorize the same prompts" }, correct: "B", explanation: "SDD creates input consistency: the root cause of inconsistent AI output is inconsistent input. Different team members describe the same feature differently, with different constraints and different context quality. SDD's solution: shared specifications reviewed by the team ensure every AI session for the same feature starts with identical, reviewed requirements. Consistent input = consistent output. Chapter 16: spec-driven development standardizes the quality floor for AI-assisted development across the entire team." },
      ]
    }
  ]
};
