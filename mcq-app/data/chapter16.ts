  ...
  ,
    {
      ...
      mcqs: [
        ...
        { id: 2, question: "What are the Three Levels of SDD and what does each level address?", options: { A: "Design, Development, Deployment — the three phases of software delivery", B: "Feature Spec (what to build), Technical Spec (how to build it), and Task Spec (granular implementation steps) — three levels of specification detail serving different audiences and purposes in the development process", C: "Requirements, Architecture, and Testing — three categories of documentation", D: "Junior, Mid, and Senior — three skill levels for developers using SDD" }, correct: "B", explanation: "Three SDD levels: (1) Feature Spec — 'what' the system should do from user/product perspective; (2) Technical Spec — 'how' the system implements it (architecture, data models, APIs); (3) Task Spec — granular implementation steps the AI agent follows. Each level serves a purpose: feature specs for stakeholder alignment, technical specs for architectural decisions, task specs for reliable AI execution. Chapter 16 uses all three in the four-phase workflow." },
        ...
        { id: 12, question: "What distinguishes a Feature Spec from a Technical Spec in the three-level SDD framework?", options: { A: "Feature Specs are for managers; Technical Specs are for developers", B: "Feature Spec: what the system does from user perspective (behavior, acceptance criteria, user stories). Technical Spec: how it's built (architecture, data models, API contracts, implementation approach). Feature Spec answers 'what/why'; Technical Spec answers 'how'", C: "Feature Specs are written in Word; Technical Specs are in Markdown", D: "There is no meaningful distinction — both describe the same things" }, correct: "B", explanation: "Level distinction: Feature Specs describe observable behavior — 'users can reset their password via email link within 10 minutes.' Technical Specs describe implementation — 'password reset token stored in Redis with 10-minute TTL, rotated on use, invalidated after successful reset.' Feature Spec is stakeholder-reviewable behavior; Technical Spec is developer-reviewable implementation contract. SDD uses both because different audiences need different specification levels." },
