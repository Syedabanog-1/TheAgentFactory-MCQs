...
export const chapter18: Chapter = {
  ...
  ,
    {
      id: 9,
      title: "Scenario-Based Mastery — Teams, CI/CD & Advanced Configuration",
      mcqs: [
        { id: 1, question: "Your team has three projects: a frontend, a backend, and a shared library. Each has different coding conventions. How does the CLAUDE.md hierarchy solve this without one monolithic file?", options: { A: "Create separate Claude Code installations for each project", B: "Use the CLAUDE.md hierarchy — each project root has its own CLAUDE.md with project-specific conventions, while a parent or user-level CLAUDE.md holds organization-wide standards. Claude merges them: local overrides global, global applies when no local rule exists", C: "Maintain one massive CLAUDE.md that lists all conventions for all projects", D: "Conventions can only be specified at session start via prompts, not in files" }, correct: "B", explanation: "CLAUDE.md hierarchy: organization-wide CLAUDE.md (shared conventions) + project-level CLAUDE.md (project-specific conventions) + directory-level CLAUDE.md (component-specific rules). Claude merges them with local specificity winning over global defaults. Frontend CLAUDE.md can specify React patterns without affecting backend Python conventions. This hierarchy prevents the 'one massive file' problem while maintaining organization-wide standards." },
        { id: 2, question: "You want different rules for TypeScript files vs. Python files in the same project. Claude should use strict typing rules for .ts files but not for .py files. What configuration mechanism enables this?", options: { A: "Create separate Claude Code profiles for each file type", B: "Path-specific rules with glob patterns — configure `*.ts` pattern rules for TypeScript and `*.py` pattern rules for Python in CLAUDE.md. Glob patterns allow file-type-specific behavior within a single project configuration", C: "This is not possible — Claude applies the same rules to all files", D: "Use separate CLAUDE.md files in the TypeScript and Python directories" }, correct: "B", explanation: "Glob pattern rules: `**/*.ts` → TypeScript-specific constraints (strict types, no any), `**/*.py` → Python-specific conventions (type hints preferred but not required). Path-specific rules in CLAUDE.md let you encode the project's actual per-language standards without separate config files. Chapter 18: glob patterns are the mechanism for file-type-aware and directory-specific Claude behavior." },
        { id: 3, question: "You've built a Skill using frontmatter. What does the frontmatter section of a Skill file define?", options: { A: "The Skill's implementation logic in a declarative format", B: "Skill metadata — name, description, trigger conditions, and usage guidance. Frontmatter tells Claude when to invoke the Skill (what conditions match it), what it does (description), and how to use it (parameters). The frontmatter is the Skill's contract; the body is its implementation", C: "Frontmatter is a documentation section ignored by Claude during execution", D: "Frontmatter defines the Skill's pricing and licensing terms" }, correct: "B", explanation: "Skill frontmatter is the discovery and contract layer: name (how it's referenced), description (what it does — used by Claude to match Skill to task), trigger conditions (when Claude should suggest or use this Skill), and parameter documentation. Without frontmatter, a Skill file is just a markdown document. With frontmatter, it becomes a discoverable, invocable capability that Claude can use autonomously or by user request." },
        { id: 4, question: "When should you use Plan Mode vs. Direct Execution for an AI-assisted task?", options: { A: "Always use Plan Mode — planning prevents mistakes", B: "Plan Mode for complex, multi-step, potentially destructive tasks where reviewing the plan before execution prevents expensive mistakes. Direct Execution for well-understood, simple, reversible tasks where planning overhead exceeds benefit. Match mode to task risk level", C: "Always use Direct Execution — planning slows development", D: "Plan Mode is for production systems; Direct Execution is for development" }, correct: "B", explanation: "Plan Mode vs. Direct Execution: Plan Mode generates the execution plan first for human review — valuable when wrong execution means rework, data loss, or deployment failures. Direct Execution proceeds immediately — valuable for low-risk, well-specified tasks. The decision: 'would I want to review this plan before it runs?' If yes → Plan Mode. If no → Direct Execution. Chapter 18: Plan Mode is trust-but-verify; Direct Execution is trust-and-go." },
        { id: 5, question: "You're implementing a complex feature with 10 subtasks. After 6 subtasks, Claude seems to drift from the original specification. What iterative refinement technique helps?", options: { A: "Restart the session from the beginning with a clearer prompt", B: "Mid-session refinement: explicitly redirect by stating the current deviation and the specification requirement. 'Current implementation is drifting toward X. The spec requires Y. Please realign.' Iterative refinement is the practice of course-correcting during execution rather than restarting from scratch", C: "Accept the drift — AI judgment may be better than the original spec", D: "Add more detail to CLAUDE.md and hope Claude self-corrects" }, correct: "B", explanation: "Iterative refinement: long sessions accumulate context drift — later steps may be influenced more by intermediate outputs than the original spec. Refinement technique: explicitly state the deviation and the correct direction. 'The current approach is doing X; the spec requires Y, here's why.' This is mid-stream correction without session restart. Chapter 18: iterative refinement is the skill of steering AI execution toward the intended outcome across multi-step implementations." },
        { id: 6, question: "Your CI/CD pipeline should run automated code review using Claude. The review should flag security vulnerabilities before merges. How does Claude Code integrate into the CI pipeline?", options: { A: "CI/CD integration with Claude is not supported", B: "Claude Code CLI can be invoked non-interactively in CI pipelines — `claude --no-interactive --print 'Review this diff for security vulnerabilities'`. Results feed into PR checks. The review runs automatically on each PR, flagging issues before merge without developer manual invocation", C: "Claude Code requires a human operator to initiate each review session", D: "CI integration requires a special enterprise license from Anthropic" }, correct: "B", explanation: "Claude Code CI integration: the CLI supports non-interactive mode (`--no-interactive`, `--print`) for automated pipelines. A CI step invokes Claude with the PR diff as context, receives review output, and posts findings as PR comments or blocks merge on critical findings. Chapter 18's CI/CD pattern: Claude becomes an automated reviewer in the pipeline — same security review quality as manual, but automated and consistent for every PR." },
        { id: 7, question: "What is a 'Multi-Pass Review Architecture' and when does it improve over single-pass review?", options: { A: "Running the same review prompt multiple times and taking the average", B: "Sequential focused passes: Pass 1 reviews security, Pass 2 reviews performance, Pass 3 reviews code style. Each pass focuses attention on one dimension, catching issues a single 'review everything' pass misses due to attention dispersion across too many concerns simultaneously", C: "Having multiple AI models each review the code independently", D: "A multi-pass review is identical to a single-pass review with a longer prompt" }, correct: "B", explanation: "Multi-pass review: a single 'review this code' prompt divides attention across security, performance, style, and logic simultaneously — each dimension gets incomplete attention. Sequential focused passes: Pass 1 with security context only → deep security findings. Pass 2 with performance focus only → performance issues. Pass 3 with style → consistency violations. Total coverage improves because each pass applies full attention to one dimension. Chapter 18: complex review tasks benefit from structured multi-pass architecture." },
        { id: 8, question: "A developer's session crashes midway through a complex 4-hour implementation. What session management capability allows them to resume without losing progress?", options: { A: "Sessions cannot be resumed — the developer must restart from scratch", B: "Session resume (`claude --continue` or session fork) — Claude Code supports resuming interrupted sessions from their last state, or forking a session to explore alternative approaches from a checkpoint. Combined with progress files (Principle 5 from Chapter 17), this enables reliable long-horizon work", C: "Only sessions shorter than 30 minutes can be resumed", D: "Session resume requires manually copying context from the crashed session" }, correct: "B", explanation: "Session management: `claude --continue` resumes the most recent session from its interrupted state — context, decisions, and progress intact. Session fork creates a branch from a checkpoint to explore alternatives without losing the original path. Combined with progress files, session resume handles the 4-hour implementation scenario: the session resumes, reads the progress file for current state, and continues. Chapter 18: session management is the operational resilience layer for long-running AI-assisted work." },
        { id: 9, question: "Your organization has 50 developers using Claude Code. You want to enforce that ALL sessions across ALL developers follow the same security standards. What is the most scalable configuration approach?", options: { A: "Train all 50 developers to remember to specify security standards in every session", B: "Organization-level CLAUDE.md in a shared repository that all developer workstations sync — security standards defined once, applied to every session automatically. The hierarchy ensures local project-level configs can extend but not override security requirements", C: "Send a weekly email reminder about security standards", D: "Create a mandatory onboarding quiz about security standards" }, correct: "B", explanation: "Scalable configuration: 50 developers × manual specification = inconsistency. Shared CLAUDE.md in a repository (cloned to every workstation, or managed via organization-wide Claude Code settings) applies standards automatically. Organization-level configuration is the Claude Code equivalent of organization-wide linting rules or security policies — defined once, enforced everywhere, maintained centrally. Chapter 18: team configuration at scale requires hierarchy and shared configuration management." },
        { id: 10, question: "A developer is implementing a database migration and wants to review Claude's plan before any database changes are made. What workflow does Chapter 18 recommend?", options: { A: "Direct Execution — migrations are straightforward and don't need review", B: "Plan Mode — for database migrations (irreversible, affects production data), generate the plan first: review migration steps, verify rollback procedures, confirm affected tables. Only approve execution after plan review. This is precisely the scenario Plan Mode was designed for", C: "Run the migration in a test environment first, then ask Claude to repeat it in production", D: "Plan Mode is only for code changes, not database operations" }, correct: "B", explanation: "Plan Mode for database migrations: migrations are often irreversible (ALTER TABLE, DROP COLUMN, data transformations) with production data consequences. Plan Mode generates the sequence of migration steps for human review before execution. Review checklist: Are the right tables affected? Is there a rollback plan? Does the migration handle existing data correctly? Approve only after confirmation. Chapter 18: Plan Mode is mandatory for operations where wrong execution has irreversible consequences." },
        { id: 11, question: "You want Claude to format all Python files with Black formatter after every file edit automatically. How do you configure this?", options: { A: "Add a reminder in CLAUDE.md to run Black manually after edits", B: "Configure a PostToolUse hook on Edit/Write tools for Python files — the hook runs `black <edited_file>` automatically after every Python file modification. Zero developer action required; formatting is consistent and automatic for every edit", C: "Create a Skill that runs Black when invoked by the developer", D: "This requires a custom MCP server to integrate Black" }, correct: "B", explanation: "Automatic formatting via hooks: PostToolUse hook on Edit tools, filtered to `*.py` files, runs `black <file_path>` immediately after each edit. The developer never needs to remember to run Black; every Python edit is automatically formatted. Hooks turn 'remember to do this after edits' into 'this happens after every edit.' Chapter 18: hooks are the automation layer for quality-enforcing workflows that should run consistently without human trigger." },
        { id: 12, question: "What is the purpose of 'session forking' in Chapter 18's session management?", options: { A: "Creating a backup copy of the session in case of crashes", B: "Branching from a checkpoint to explore alternative implementation approaches without losing the original path — fork explores approach A (keeping the main session for approach B). If approach A fails, the main session remains intact at the fork point", C: "Splitting a session between multiple developers simultaneously", D: "Session forking creates multiple AI instances for parallel work" }, correct: "B", explanation: "Session fork = branch exploration: at decision point X (approach A or approach B?), fork the session. Fork explores approach A. If approach A fails or is suboptimal, the fork is discarded — the main session at checkpoint X is intact to explore approach B. This is git branching for AI sessions. Chapter 18: session forking enables safe exploration of alternative implementations without the cost of 'wrong choice = restart from scratch.'" },
        { id: 13, question: "A team's custom Skills aren't being invoked by Claude in the right contexts. Developers have to manually tell Claude to use the Skill every time. What frontmatter element is likely missing?", options: { A: "The Skill file is missing the implementation body", B: "Trigger conditions in the frontmatter — without explicit trigger conditions (which contexts/tasks should invoke this Skill), Claude doesn't know when to suggest or automatically use the Skill. Trigger conditions match task types to Skill capabilities, enabling automatic Skill discovery", C: "The Skill needs to be published to the agentskills.io marketplace first", D: "Skills require manual invocation — automatic triggering is not supported" }, correct: "B", explanation: "Frontmatter trigger conditions: define what task descriptions or file patterns should prompt Claude to suggest or invoke the Skill. Without triggers: Claude sees a Skill as a document, not a behavior. With triggers: 'when the user asks to review a PR' → auto-suggest the PR Review Skill. Well-defined triggers are what separate a discoverable Skill from a file Claude has to be manually told about every time. Chapter 18: frontmatter triggers are the discovery mechanism for Skills." },
        { id: 14, question: "How does Chapter 18's guidance on CI/CD integration relate to Chapter 17's Principle 7 (Observability)?", options: { A: "They are unrelated — CI/CD is about deployment, Observability is about monitoring", B: "CI/CD integration gives agent actions organizational visibility: Claude's code reviews, test executions, and quality checks produce observable artifacts (PR comments, build reports, lint results) that the entire team can see. CI/CD makes agent Observability systematic and team-wide rather than per-developer", C: "CI/CD replaces the need for Observability in development workflows", D: "Observability only applies to production systems, not CI/CD pipelines" }, correct: "B", explanation: "CI/CD as organizational observability: Chapter 17's Principle 7 says agent actions must produce visible evidence. CI/CD integration makes that evidence organizational — Claude's review findings appear as PR comments seen by the whole team, test failures appear in build logs tracked by engineering, security findings appear in audit dashboards. Solo agent observability = developer sees output. CI/CD observability = organization sees output. Chapter 18 extends P7 to team and organizational scale." },
        { id: 15, question: "A developer configures a CLAUDE.md with 50 rules. A new developer on the team can't find the rule about database migrations. What CLAUDE.md organization principle from Chapter 18 helps?", options: { A: "Add more rules to cover every edge case", B: "Use clear heading hierarchy and organized sections: ## Database Operations → ### Migration Rules. Well-organized CLAUDE.md with proper Markdown hierarchy enables both developers and Claude to quickly locate specific rules. CLAUDE.md quality as a document determines its effectiveness as a configuration file", C: "50 rules is too many — delete rules until the file is manageable", D: "Rules don't need organization — Claude reads all of them regardless of structure" }, correct: "B", explanation: "CLAUDE.md organization: a 50-rule CLAUDE.md without structure is a wall of text — both humans finding rules and Claude locating relevant constraints require scanning everything. Markdown hierarchy (## sections, ### subsections) makes rules discoverable: ## Database → ### Migrations → specific rules. Chapter 18 applies Chapter 13's Markdown principles to configuration files: structure creates navigability for humans AND improves Claude's ability to find and apply the most relevant rules to each task." },
      ]
    }
  ]
};
